...
# Security group для кластера (EKS створює додаткові автоматично)
resource "aws_security_group" "cluster" {
  name        = "${var.cluster_name}-cluster-sg"
  description = "Cluster security group"
  vpc_id      = element(var.subnet_ids, 0) != "" ? split("/", var.subnet_ids[0])[0] : null
  # ↑ Хак не потрібен: краще передати VPC ID окремо.
}
